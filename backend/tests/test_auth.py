...
@pytest.mark.asyncio
async def test_public_register_cannot_create_researcher(client: AsyncClient):
    resp = await client.post("/api/auth/register", json={
        "email": "researcher-public@example.com",
        "password": "securepass1",
        "full_name": "Researcher Public",
        "role": "researcher",
    })
...
@pytest.mark.asyncio
async def test_closed_registration_requires_admin(client: AsyncClient, test_user, admin_user, monkeypatch):
    monkeypatch.setattr(settings, "OPEN_REGISTRATION", False)

    reviewer_resp = await client.post(
        "/api/auth/register",
        json={"email": "closed-reviewer@example.com", "password": "securepass1", "role": "reviewer"},
    ...
    admin_resp = await client.post(
        "/api/auth/register",
        json={"email": "closed-researcher@example.com", "password": "securepass1", "role": "researcher"},
