...
@pytest.mark.asyncio
async def test_public_register_cannot_create_elevated_user(client: AsyncClient):
    resp = await client.post("/api/auth/register", json={
...
@pytest.mark.asyncio
async def test_closed_registration_requires_admin_requester(client: AsyncClient, test_user, monkeypatch):
    from backend.app.config import settings

    monkeypatch.setattr(settings, "OPEN_REGISTRATION", False)
    resp = await client.post(
        "/api/auth/register",
        json={"email": "closed@example.com", "password": "password123", "role": "reviewer"},
...
@pytest.mark.asyncio
async def test_admin_can_register_user_when_registration_closed(client: AsyncClient, admin_user, monkeypatch):
    from backend.app.config import settings

    monkeypatch.setattr(settings, "OPEN_REGISTRATION", False)
    resp = await client.post(
        "/api/auth/register",
        json={"email": "invited@example.com", "password": "password123", "role": "researcher"},
