...
# Report summaries and exports are gated behind authentication (consistent
# with the /api/exports/* endpoints), so successful requests carry an auth header.

@pytest.mark.asyncio
async def test_summary_report_requires_auth(client: AsyncClient, sample_data):
    """Reading aggregate report data requires a logged-in user."""
